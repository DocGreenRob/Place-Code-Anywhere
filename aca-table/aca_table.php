...
class aca_table {
	...
	public function __construct($plugin_id, $object) {
		...
		if ($object instanceof _aca_table_obj) $this->object = $object; else return false;
		
		if ($GLOBALS['wpdb']) $this->wpdb = $GLOBALS['wpdb']; else return false;
		
		$this->user_options_var = aca_table_func::safe_plugin_id($this->plugin_id) .'_table_settings';
	...
	public function user_settings_form() {
		...
		foreach ($this->object as $pname=>$propertie) {
			...
			if ($propertie->table_view_change !== false) {
				...
				$hack .= new aca_checkbox_input(array('name'=>"wp_screen_options[value][columns][$propertie->col_name]", 'value'=>$value, 'value_to_select'=>1)) .' '. $propertie->name;
	...
	public function user_settings_save($new_settings) {
		...
		if ($_GET[aca_table_page_filter::VAR_NAME] && $new_settings['settings']['rows_per_page'] != $old_settings['settings']['rows_per_page']) aca_table_func::change_referer(aca_table_page_filter::VAR_NAME); // if number of rows per page changed switch to first page
	 ...
	 * 
	 * @param $object - instance of _aca_table_filter
	 */
	public function reg_filter($object) {
		
		if ($object instanceof _aca_table_filter) $this->filters[] = $object;
	 ...
	 * 
	 * @param $object - instance of _aca_table_action
	 */
	public function reg_bulk_action($object) {
	
		if ($object instanceof _aca_table_action) $this->bulk_actions[] = $object;
	...
	protected function table_actions() {
		...
		if ($this->enable_bulk_actions) {
			...
			$this->bulk_actions[] = new aca_table_delete_selected($this->plugin_id);
			...
			$select_action = new aca_select_input(array('name'=>'action', 'value_to_select'=>$actions));
		...
		$object_name = new aca_hidden_input(array('name'=>'object', 'value'=>get_class($this->object)));
		
		if ($this->last_record) $last_record = new aca_hidden_input(array('name'=>'last_record', 'value'=>1)); else $last_record = false;
	...
	public function reg_row_action($object) {
		...
		if ($object instanceof _aca_table_action) $this->row_actions[] = $object;
	...
	public function html() {
		...
		$res .= '<form method="post" action="'. $this->post_handler .'" id="aca_includes_table_form">'.PHP_EOL;

		$res .= new aca_hidden_input(array('name'=>'wp_nonce', 'value'=>wp_create_nonce('aca_table_post')));
				
		$res .= $table_actions;
		
		$res .= '<table class="widefat" cellspacing="0" id="aca_includes_table">'.PHP_EOL;
	...
	protected function table_rows() {
		...
		if ($this->enable_page_filter) {
			...
			$this->page_filter = new aca_table_page_filter($this->plugin_id);
		...
		if (!empty($data)) {
			...
			foreach ($data as $row) {
				...
				if ($this->enable_bulk_actions) {
					...
					$res .= '<th scope="row" class="check-column"><input type="checkbox" name="checked['. $row_id .']" value="" class="aca_table_checkbox" /></th>'.PHP_EOL;
				...
				foreach ($this->object as $propertie=>$definition) {
					...
					if ($definition->table_view) {
						...
						if ($propertie == 'title') {
							...
							$res .= '<td><a href="'. $propertie_link . '?' . aca_table_func::change_qstring('id',$row_id) .'" class="row-title">'. $definition->val .'</a>'. $this->row_actions($this->object, $row) .'</td>'.PHP_EOL;
	...
	protected function row_actions($object, $row_data) {
		...
		$row_actions[] = new aca_table_delete_row($this->plugin_id, $this->get_handler);
	...
	protected function row_edit() {
		...
		if (is_numeric($id) || $id == self::NEW_MARKER) {
			...
			if ($id == self::NEW_MARKER) {
				...
				$action = new aca_table_new_row($this->plugin_id);
								
			} else {
			
				$action = new aca_table_update_row($this->plugin_id);
			...
			$res .= '<div id="any_code_anywhere" class="postbox" style="width: 100%; margin-bottom: 0px;">'.PHP_EOL;
			...
			$res .= new aca_hidden_input(array('name'=>'object', 'value'=>get_class($this->object)));
			
			$res .= new aca_hidden_input(array('name'=>'propertie[id]', 'value'=>$id));
	
			$res .= '<table class="form-table" id="aca_inc_options_table">'.PHP_EOL;
			...
			$res .= '<script type="text/javascript">jQuery("#any_code_anywhere .hndle").click(function(){jQuery("#any_code_anywhere .inside").toggle()})</script>';
	...
	public function js_code() {
		...
		$js_code .= 'jQuery("#aca_includes_table_form").submit(function() {
	if (jQuery("#aca_includes_table .aca_table_checkbox:checked").length > 0) {
		var selected_option = jQuery("#aca_includes_table_form select[name=\'action\']").attr("value");
		if (selected_option) {
			if (selected_option == "aca_table_delete_selected") return confirm("'. __('Delete selected?', $this->plugin_id) .'"); else	return true;
		} else return false;
	} else {
		return false;
	}
		...
		$js_code .= 'jQuery("#aca_includes_table_form select[name=\'action\']").change(function() {
	jQuery("#aca_includes_table_form select[name=\'action\']").attr("value", jQuery(this).attr("value"));
});'.PHP_EOL;
		
		$js_code .= 'jQuery("#aca_includes_table_form .new_include").click(function() {
	location.href = "'. get_option('siteurl') .'/wp-admin/admin.php?page='. $_GET['page'] .'&id='. self::NEW_MARKER .'";
});'.PHP_EOL;
		
		$js_code .= 'jQuery("#aca_includes_table .manage-column :checkbox").click(function() {
	if (jQuery(this).attr("checked")) {
		jQuery("#aca_includes_table .aca_table_checkbox").attr("checked",true);
	} else {
		jQuery("#aca_includes_table .aca_table_checkbox").removeAttr("checked");
	}
});'.PHP_EOL;

		$js_code .= 'jQuery("#aca_includes_table .delete_row").click(function() {
