 ...
 * @author Max Bond
 * 
 * aca Table support functions class
 *
 */
class aca_table_func {
	...
	/**
	 * Returns aca table installation folder url
	 * 
	 * @return string
	 */
	...
	public static function css_js_load() {
		...
		wp_enqueue_style('aca_table_css', $folder .'_css/aca_table.css');
		
		wp_enqueue_style('aca_obj_css', $folder . '_css/aca_obj.css');
		
		if (array_key_exists('id', $_GET) && $_GET['id']) {
			
			wp_enqueue_style('aca_table_tooltip_css', $folder .'_js/tooltip/tooltip.css');
	
			wp_enqueue_script('aca_table_tooltip_js', $folder .'_js/tooltip/tooltip.js');
	...
	public static function change_ref_after_del($id_col_name, $id) {
		...
		aca_table_func::change_referer('&'. $id_col_name .'='.$id, ''); // remove deleted id from query string
	
		if (!(isset($_GET['last_record']) || isset($_POST['last_record']))) return; // continue if deleted record is last

		$page = strstr($_SERVER['HTTP_REFERER'], aca_table_page_filter::VAR_NAME);
		...
		if ($page_num > 1) { // if this is not first page reset page number to 1
			...
			aca_table_func::change_referer('&'.$page, '');
					
		} else { // if this is first page reset location filter var
				
			$location = strstr($_SERVER['HTTP_REFERER'], aca_table_location_filter::VAR_NAME);
			...
			if ($location) {
				...
				aca_table_func::change_referer('&'.$location, '');
	...
	public static function page_selectors() {
		...
		$res = aca_include_obj::page_selectors();
		
		foreach ($wp_post_types as $post_type) {
			
			if (!in_array($post_type->name, any_code_anywhere::$restricted_post_types)) {
				
				$res += array($post_type->name.'_all' => __('All', any_code_anywhere::ID).' '.$post_type->labels->name);
				
				$res += array('post_type_archive_'.$post_type->name =>  __('Archive', any_code_anywhere::ID).': '.$post_type->labels->name);
		...
		foreach ($wp_taxonomies as $taxonomy) {
			...
			if (!in_array($taxonomy->name, any_code_anywhere::$restricted_taxonomies)) {
				
				$res += array($taxonomy->name.'_all' => __('All', any_code_anywhere::ID).' '.$taxonomy->labels->name);
		...
		$plugin_id = isset($plugin_id) ? $plugin_id : '';
	...
	public static function select_taxonomy($taxonomy) {
		...
		return isset($output_array) ? $output_array : '';
	...
	public static function wp_user_roles() {
		...
		$roles['aca_visitor'] = __('Visitor', any_code_anywhere::ID);
