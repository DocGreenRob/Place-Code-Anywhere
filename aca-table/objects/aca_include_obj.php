 ...
 * 
 */
class aca_include_obj extends _aca_table_obj {
	...
	public function table() {
		...
		return $wpdb->prefix . any_code_anywhere::ID;
	...
	/**
 	 ...
 	 * Refer to _aca_table_obj_properties class for field options description
 	 * 
 	 */
	protected function init() {
		...
		$this->id->col_index = _aca_table_obj_propertie::PRIMARY;
		$this->id->input = new aca_hidden_input(array('name'=>$this->input_name($this->id->col_name)));
		...
		$this->title->input = new aca_text_input(array('name'=>$this->input_name($this->title->col_name), 'class'=>'text_input', 'maxlength'=>255));
		...
		$this->status->input = new aca_select_input(array('name'=>$this->input_name($this->status->col_name), 'value_to_select'=>$this->status->input_values, 'class'=>'select_input'));
		$this->status->help = __('Active - code will be displayed, Disabled - code will not be displayed', self::$plugin_id);
		$this->status->conv = new aca_select_conv($this->status, self::$plugin_id);
		...
		$this->location->input = new aca_select_input(array('name'=>$this->input_name($this->location->col_name), 'value_to_select'=>$this->location->input_values, 'class'=>'select_input', 'id'=>'select_location'));
		...
		$this->location->conv = new aca_select_conv($this->location, self::$plugin_id);
		...
		$this->widget_title->input = new aca_text_input(array('name'=>$this->input_name($this->widget_title->col_name), 'maxlength'=>255));
		$this->widget_title->conv = new aca_text_conv();
		...
		$this->priority->input = new aca_text_input(array('name'=>$this->input_name($this->priority->col_name), 'class'=>'digit_input', 'maxlenght'=>4));
		$this->priority->help = __('Determines display order of includes in the same location', self::$plugin_id);
		$this->priority->conv = new aca_priority_conv();
		...
		$this->inc_pages->input = new aca_wp_page_select_input(array('name'=>$this->input_name($this->inc_pages->col_name), 'id'=>'inc_pages_select', 'type'=>'include'));
		...
		$this->inc_pages->conv = new aca_select_page_conv($this->inc_pages, self::$plugin_id);
		...
		$this->exc_pages->input = new aca_wp_page_select_input(array('name'=>$this->input_name($this->exc_pages->col_name), 'id'=>'exc_pages_select', 'type'=>'exclude'));
		...
		$this->exc_pages->conv = new aca_select_page_conv($this->exc_pages, self::$plugin_id);
		...
		$this->hide_from->input = new aca_checkbox_multi_input(array('name'=>$this->input_name($this->hide_from->col_name), 'value_to_select'=>aca_table_func::wp_user_roles()));
		...
		$this->hide_from->conv = new aca_checkbox_multi_conv('', self::$plugin_id, aca_table_func::wp_user_roles());
		...
		$this->code_align->input = new aca_select_input(array('name'=>$this->input_name($this->code_align->col_name), 'value_to_select'=>$this->code_align->input_values, 'class'=>'select_input'));
		$this->code_align->conv = new aca_text_conv($this->code_align, self::$plugin_id);
		...
		$this->code->input = new aca_textarea_input(array('name'=>$this->input_name($this->code->col_name), 'class'=>'textarea_input', 'id'=>'inc_code'));
	...
	protected function check_input_data() {
		...
		if (!$this->title->val) {
			...
			new aca_table_sys_msg(__('You must input Description field!', self::$plugin_id));
			
			return false;
		
		} elseif (!intval($this->priority->val)) {
		
			new aca_table_sys_msg(__('Priority number must be integer!', self::$plugin_id));
			
			return false;
				
		} elseif (!$this->inc_pages->val) {
		
			new aca_table_sys_msg(__('You must select at least one page where to place included code!', self::$plugin_id));
			
			return false;
				
		} elseif (!$this->code->val) {
		
			new aca_table_sys_msg(__('You must enter included code!', self::$plugin_id));
	...
	public function change_status($status) {
		...
		if ($this->id->col_name && $this->id->val && $this->status->col_name && $status) {
			...
			if ($wpdb->update($this->table(), array($this->status->col_name=>$status), array($this->id->col_name=>$this->id->val)) !== false) {
				...
				new aca_table_sys_msg(__('Status changed', self::$plugin_id));
	...
	public function js() {
		...
		$this->widget_title->input->value = sanitize_text_field($this->widget_title->val);
		
		$this->code_align->input->value = sanitize_text_field($this->code_align->val);
		
		$zcbpath = aca_table_func::folder_url() .'_js/zeroclipboard/';
		
		$zcbptext = __('Copy to clipboard', self::$plugin_id);
		
		$manual_inc_code = '&lt;?php if ( method_exists(\'any_code_anywhere\', \'manual_inc\') ) any_code_anywhere::manual_inc('. $_GET['id'] .'); ?&gt;';
