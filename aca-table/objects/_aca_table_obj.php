 ...
 * @author Max Bond
 * 
 * Abstract _aca_table_obj class. Describes basic methods for all _aca_table_class children classes
 *
 */
abstract class _aca_table_obj {
	...
	public function __construct($plugin_id, $object_id = false, $conv_type = false) {
		...
		foreach ($this as $pname=>$propertie) {
			...
			$this->$pname = new _aca_table_obj_propertie(); // create object properties
	...
	public function save() {
		...
		if ($this->check_input_data()) { // valid user input
			...
			if ($this->id->val && $this->id->val != aca_table::NEW_MARKER) { // update
				
				if ($wpdb->update($this->table(), $this->values_array(), array($this->id->col_name=>$this->id->val)) !== false) { // ok	
		
					new aca_table_sys_msg(__('Record updated', self::$plugin_id)); // message to user
					...
					new aca_table_sys_msg(__('DB error:', self::$plugin_id).' '.$wpdb->last_error);
					
					return false;
					
				}	
					
			} elseif ($this->id->val && $this->id->val == aca_table::NEW_MARKER) { // insert
				...
				if ($wpdb->insert($this->table(), $this->values_array()) !== false) { // ok				
					...
					new aca_table_sys_msg(__('Record inserted', self::$plugin_id)); // message to user
	...
	public function delete() {
		...
		if ($this->id->col_name && $this->id->val) { // id supplied
			...
			if ($wpdb->query('DELETE FROM '. $this->table() .' WHERE '. $this->id->col_name .' = '. $this->id->val) !== false) { // ok
				...
				new aca_table_sys_msg(__('Record(s) deleted', self::$plugin_id)); // msg for user
				
				return true;
			
			} else { // error
				
				new aca_table_sys_msg(__('DB error:', self::$plugin_id).' '.$wpdb->print_error()); // error msg
	...
	protected function table_structure() {
		...
		foreach ($this as $key=>$prop) {
			...
			if ($prop->col_index == _aca_table_obj_propertie::PRIMARY) {
			
				$primaries[] = $prop->col_name;
			
			} elseif ($prop->col_index == _aca_table_obj_propertie::UNIQUE) {
			
				$uniques[] = $prop->col_name;
			
			} elseif ($prop->col_index == _aca_table_obj_propertie::INDEX) {
	...
	protected function save_input() {
		...
		$_SESSION['aca_table_tmp_data'] = $this->values_array();
	...
	protected function get_saved_input() {
		...
		$_SESSION['aca_table_tmp_data'] = isset($_SESSION['aca_table_tmp_data']) ? $_SESSION['aca_table_tmp_data'] : NULL;

		
		if (array_key_exists('id', $_GET) && $_GET['id'] == aca_table::NEW_MARKER && is_array($_SESSION['aca_table_tmp_data']) && !empty($_SESSION['aca_table_tmp_data'])) {
		
			$this->load_values_from_array($_SESSION['aca_table_tmp_data'], 'db2php');
		
			unset($_SESSION['aca_table_tmp_data']);
