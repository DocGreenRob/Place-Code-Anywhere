 ...
 *
 */
class aca_table_location_filter extends _aca_table_filter {
	...
	public function controls() {
		...
		$inc_obj = new aca_include_obj($this->plugin_id);
		...
		foreach ($filters as $filter_key=>$filter_name) { // sub filter cycle
			...
			if ($count[0][0] > 0) {
				...
				$link = aca_table_func::change_qstring(self::VAR_NAME, $filter_key); // change filter id
				
				$link = $_SERVER['PHP_SELF'].'?'.aca_table_func::change_qstring(aca_table_page_filter::VAR_NAME, '', $link); // reset page to number 1
