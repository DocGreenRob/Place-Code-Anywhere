<?php
/*
Plugin Name: Place Code Anywhere
Plugin URI: http://www.snippet.farm
Description: This plugin allows you to insert html, css, javascript and PHP into WordPress pages.
Version: 1.0
Author: Robert Green
Author URI: http://www.justinhandley.com
*/

if (defined('ABSPATH')) { // makes shure that the following functions will be run inside WordPress only

	require_once 'aca-table/aca_table_func.php';
		
	require_once 'aca-table/aca_table_load.php'; // loads aca_table library
	
	require_once 'any_code_anywhere_widget.php'; // loads inc manager widget class
	
	// Hooks 
	
	register_activation_hook(__FILE__, array( 'any_code_anywhere', 'activate' )); // registers activation function

	add_action('widgets_init', array( 'any_code_anywhere', 'widget_incs' )); // widgets initialization both for admin and public pages
	
	add_shortcode('include', array( 'any_code_anywhere', 'shortcode_incs' )); // shortcode init
	
	add_shortcode('INCLUDE', array( 'any_code_anywhere', 'shortcode_incs' ));
	
	if (is_admin()) { // admin hooks
		
		any_code_anywhere::load_language();
		
		add_action('admin_menu', array( 'any_code_anywhere', 'reg_menu' ));
		
		add_filter('plugin_action_links_'.plugin_basename(__FILE__), array( 'any_code_anywhere','reg_control_links' ));
		
		add_filter('set-screen-option', array( 'any_code_anywhere', 'screen_options_save' ), 10, 3);
					
	} else { // public hooks
		
		add_action('wp_head', array( 'any_code_anywhere', 'cur_page' ));
		
		add_action('wp_head', array( 'any_code_anywhere', 'header_incs' ));
	
		add_action('wp_footer', array( 'any_code_anywhere', 'footer_incs' ));
		
		add_action('loop_start', array( 'any_code_anywhere', 'b_page_content_incs' ));
	
		add_action('loop_end', array( 'any_code_anywhere', 'a_page_content_incs' ));
	
		add_filter('the_content', array( 'any_code_anywhere', 'b_post_content_incs' ));
		
		add_filter('the_content', array( 'any_code_anywhere', 'a_post_excerpt_incs' ));
	
		add_filter('the_content', array( 'any_code_anywhere', 'a_post_content_incs' ));
		
		add_filter( 'comment_text', array( 'any_code_anywhere', 'parse_shortcodes_comment' ));
...
if (class_exists('any_code_anywhere', false)) return; // if class allready loaded returns control to the main script
 ...
 * 
 */
class any_code_anywhere {

	const ID = 'any_code_anywhere'; // Plugin ID, also used as a Text Domain name
		
	const NAME = 'Any Code Anywhere'; // Plugin name
	...
	protected static $table; // @var self::$table aca_table
	
	protected static $object; // @var self::$object aca_include_obj
	 ...
	 * 
	 * @return object instance of _aca_table_obj
	 */
	public static function object() {
		
		if (!self::$object) self::$object = new aca_include_obj(self::ID);
	 ...
	 * 
	 * @return object instance aca_table
	 */
	public static function table() {
		...
		if (!self::$table) {
			...
			$table = new aca_table(self::ID, $inc_obj);
			
			$table->get_handler = plugins_url( 'aca-table/aca_get.php', __FILE__ );
	
			$table->post_handler = plugins_url( 'aca-table/aca_post.php', __FILE__ );
	
			$table->reg_filter(new aca_table_status_filter(self::ID)); // register status filter
			
			$table->reg_filter(new aca_table_location_filter(self::ID)); // register location filter

			$table->reg_filter(new aca_table_search_title_filter(self::ID, $inc_obj)); // register search filter
			
			$table->reg_bulk_action(new aca_table_activate_selected(self::ID)); // register bulk activate table action
		
			$table->reg_bulk_action(new aca_table_disable_selected(self::ID)); // register bulk deactivate table action
		
			$table->reg_row_action(new aca_table_change_status(self::ID, $table->get_handler)); // register single row action
	...
	public static function reg_menu() {
		...
		add_menu_page('Custom Code', __('Custom Code', self::ID), $access_level, 'any-code-anywhere', array(__CLASS__,'main_page'), plugins_url( 'aca-table/menu-icon.gif', __FILE__ ));
		
		self::$plugin_page = add_submenu_page('any-code-anywhere', self::NAME, __('Your Codes', self::ID), $access_level, 'any-code-anywhere', array(__CLASS__,'main_page'));
		
		add_submenu_page('any-code-anywhere', self::NAME, __('Add New', self::ID), $access_level, 'any-code-anywhere&amp;id=_new_', array(__CLASS__,'main_page'));
		
		add_submenu_page('any-code-anywhere', self::NAME, __('Settings', self::ID), $access_level, 'any-code-anywhere-settings', array(__CLASS__,'settings_page'));
		...
		if (isset($_GET["page"]) && $_GET["page"] == 'any-code-anywhere') {
			
			aca_table_func::css_js_load(); // css and js for settings page
	...
	public static function help($help_content) {
		...
		$help_content[self::$plugin_page] = '<a href="http://www.snippet.farm/">'. __('Any Code Anywhere Homepage', self::ID) .'</a>';
	...
	public static function reg_control_links($links) {
		...
		$links[$index] = '<a href="admin.php?page=any-code-anywhere&amp;deactivate=true">'. __('Deactivate') .'</a>'; // changes default plugin deactivation link // now it points to my custom plugin deactivation page
	...
	public static function main_page() {
		...
		if (key_exists('deactivate', $_GET) && $_GET['deactivate'] == 'true') {
			...
			$res .= '<ul class="subsubsub"><li>'. __('Need help? Visit', self::ID) .' <a href="http://www.aca.ru/code-insert-manager-wordpress-plugin/">'. __('Plugin Homepage', self::ID) .'</a></li></ul>'.PHP_EOL;
	...
	public static function settings_page() {
		...
		$options['shortcodes_in_comments'] = isset($options['shortcodes_in_comments']) ? $options['shortcodes_in_comments'] : '';
		$res .= '<input type="checkbox" name="'. self::ID.'[shortcodes_in_comments]" '. checked($options['shortcodes_in_comments'], 'on', false) .' /> '. __('Enable shortcodes in comments. This option only affects shortcodes generated by Any Code Anywhere', self::ID).PHP_EOL;
		...
		//$res .= '<ul class="subsubsub"><li>'. __('Need help? Visit', self::ID) .' <a href="http://www.aca.ru/2009/12/06/824/">'. __('Plugin Homepage', self::ID) .'</a></li></ul>'.PHP_EOL;
		...
		$res .= '<li>'. __('Rate it on the official Plugin Directory', self::ID) .': <a href="http://wordpress.org/extend/plugins/any-code-anywhere/" target="_blank">http://wordpress.org/extend/plugins/any-code-anywhere/</a></li>'.PHP_EOL;
	...
	protected static function deactivate_page() {
		...
		$res .= '<input type="hidden" name="wp_nonce" value="'. wp_create_nonce('aca_table_post') .'"/>'.PHP_EOL;
	...
	public static function deactivation() {
		...
		if (isset($_POST['deactivate']) && $_POST['deactivate'] == 'deactivate' || $_POST['deactivate'] == 'deactivate_and_clean') { // process deactivation options
			...
			if ($_POST['deactivate'] == 'deactivate') { // simple deactivation
				...
				$wpdb->query('DELETE FROM '. $wpdb->options ." WHERE option_name LIKE '%any_code_anywhere%'"); // delete all plugin entries in options table
				
				$wpdb->query('DELETE FROM '. $wpdb->usermeta ." WHERE meta_key = '". aca_table_func::safe_plugin_id(self::ID)."_table_settings'"); // delete all plugin entries in usermeta table
	...
	public static function header_incs() {
		...
		echo self::display_incs(aca_include_obj::LOC_HEADER);
	...
	public static function footer_incs() {
		...
		echo self::display_incs(aca_include_obj::LOC_FOOTER);
	...
	public static function widget_incs() {
		...
		$incs = self::select_incs(aca_include_obj::LOC_WIDGET);
		...
		if (is_array($incs) && !empty($incs)) {
			...
			foreach ($incs as $inc) {
				...
				$widget_id = 'any_code_anywhere_widget_'.$object_id;
				...
				$wp_widget_factory->widgets[$widget_id] = new any_code_anywhere_widget(self::ID, $plugin_name, $object_id, $widget_id, $widget_admin_title, $widget_public_title, $inc_pages, $exc_pages, $hide_from, $code_align, $code);
	...
	public static function b_page_content_incs() {
		...
		if ($i == 0) echo self::display_incs(aca_include_obj::LOC_B_PAGE_CONTENT);
	...
	public static function a_page_content_incs() {
		...
		if ($i == 0) echo self::display_incs(aca_include_obj::LOC_A_PAGE_CONTENT);
	...
	public static function b_post_content_incs($content) {
		...
		return self::display_incs(aca_include_obj::LOC_B_POST_CONTENT).$content;
	...
	public static function a_post_excerpt_incs($content) {
		...
		return str_replace('<span id="more-'.$post->ID.'"></span>', '<span id="more-'.$post->ID.'"></span>'.self::display_incs(aca_include_obj::LOC_A_POST_EXCERPT), $content);
	...
	public static function a_post_content_incs($content) {
		...
		return $content.self::display_incs(aca_include_obj::LOC_A_POST_CONTENT);
	...
	public static function manual_inc($id, $echo = true) {
		...
		if (!$echo) return self::display_inc($id, aca_include_obj::LOC_MANUAL);
		
		echo self::display_inc($id, aca_include_obj::LOC_MANUAL);
	...
	public static function shortcode_incs($atts, $content = null) {
		...
		return self::display_inc($id, aca_include_obj::LOC_SHORTCODE);
	 ...
	 * @param $location One of location constants stored in aca_include_obj class
	...
	protected static function display_inc($id, $location) {
		...
		if (self::$object->id->val && self::$object->location->val == $location) {
			...
			if (self::check_visibility($inc_pages, $exc_pages, $hide_from)) {
				...
				if (self::$object->status->val == aca_include_obj::STATUS_ACTIVE) {
	...
	protected static function select_incs($location) {
		...
		$res = $wpdb->get_results('SELECT * FROM '. self::$object->table() .'  WHERE '. self::$object->status->col_name .' = '. aca_include_obj::STATUS_ACTIVE .' AND ' . self::$object->location->col_name .' = '. $location .' ORDER BY '. self::$object->priority->col_name .','. self::$object->id->col_name, ARRAY_A);
	...
	public static function check_visibility($inc_pages, $exc_pages, $hide_from_role) {
		...
		if ($hide_from_role) {
			...
			if ($user->ID === 0 && in_array('aca_visitor', $hide_from_role)) return false; // check visitor role
	...
	public static function cur_page() {
		...
		if (!$page_id) {
			...
			if (is_front_page() || is_home() && !$GLOBALS['wp_query']->is_posts_page) { // front page
				...
				$page_id = array(aca_include_obj::ALL, aca_include_obj::FRONT_PAGE);
				
			} elseif (is_attachment()) { // attachment page
				
				$page_id = array(aca_include_obj::ALL, aca_include_obj::ATTACHMENT_PAGES);
				
			} elseif (is_single()) { // post page + custom post type page
				
				$post_type = get_post_type($GLOBALS['post']);
				
				$page_id = array(aca_include_obj::ALL, aca_include_obj::POST_TYPES_PAGES, $post_type.'_all', $post_type.'_'.$GLOBALS['post']->ID);
				...
				$page_id = array(aca_include_obj::ALL, aca_include_obj::POST_TYPES_PAGES, 'page_all', 'page_'.$GLOBALS['post']->ID);
				...
				$page_id = array(aca_include_obj::ALL, aca_include_obj::TAX_PAGES, $taxonomy.'_all', $taxonomy.'_'.$tax_obj->term_id);
				
			} elseif (function_exists('is_post_type_archive') && is_post_type_archive()) { // post type archive page
				
				$post_type = get_post_type($GLOBALS['post']);
				
				$page_id = array(aca_include_obj::ALL, aca_include_obj::POST_TYPE_ARCHIVE_PAGES, 'post_type_archive_'.$post_type);
				
			} elseif ($GLOBALS['wp_query']->is_posts_page) { // post type archive page for 'post' post type
				
				$page_id = array(aca_include_obj::ALL, aca_include_obj::POST_TYPE_ARCHIVE_PAGES, 'post_type_archive_post');
				
			} elseif (is_date()) { // date archive page
				
				$page_id = array(aca_include_obj::ALL, aca_include_obj::DATE_PAGES);
				
			} elseif (is_author()) { // author page
				
				$page_id = array(aca_include_obj::ALL, aca_include_obj::AUTHOR_PAGES);
				
			} elseif (is_search()) { // search page
				
				$page_id = array(aca_include_obj::ALL, aca_include_obj::SEARCH_PAGE);
				
			} elseif (is_404()) { // error 404 page
				
				$page_id = array(aca_include_obj::ALL, aca_include_obj::PAGE_404);
				
			}
			
			if (is_preview()) { // preview page
	
				array_push($page_id, aca_include_obj::PREVIEW_PAGE);
				
			}
	
			if (is_paged()) { // paged page
				
				array_push($page_id, aca_include_obj::PAGED_PAGES);
	...
	public static function code_align($code_str, $code_align) {
		...
		if ($code_align) {
			...
			switch ($code_align) {
				...
				case aca_include_obj::ALIGN_LEFT:
					
					$align = 'left';
					
					break;
					
				case aca_include_obj::ALIGN_CENTER:
					
					$align = 'center';
					
					break;
					
				case aca_include_obj::ALIGN_RIGHT:
		...
		return sanitize_text_field($code_str);
